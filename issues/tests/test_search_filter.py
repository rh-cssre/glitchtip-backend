...
class FilterTestCase(GlitchTipTestCase):
    ...
    def test_filter_by_tag(self):
        ...
        event_tag_key_browser = baker.make("issues.EventTagKey", key=tag_browser)
        event_tag_key_mythic_animal = baker.make(
            "issues.EventTagKey", key=tag_mythic_animal
        )
        event_tag_firefox = baker.make(
            "issues.EventTag", key=event_tag_key_browser, value=tag_value_firefox
        )
        event_tag_chrome = baker.make(
            "issues.EventTag", key=event_tag_key_browser, value=tag_value_chrome
        )
        event_tag_mythic_animal_firefox = baker.make(
            "issues.EventTag", key=event_tag_key_mythic_animal, value=tag_value_firefox
        )
        event_tag_mythic_animal_cthulhu = baker.make(
            "issues.EventTag", key=event_tag_key_mythic_animal, value=tag_value_cthulhu
        )

        event_only_firefox = baker.make("issues.Event", issue__project=self.project)
        issue_only_firefox = event_only_firefox.issue
        event_only_firefox2 = baker.make("issues.Event", issue=issue_only_firefox)
        event_only_firefox.tags.add(event_tag_firefox)
        event_only_firefox2.tags.add(event_tag_mythic_animal_cthulhu)

        event_firefox_chrome = baker.make("issues.Event", issue__project=self.project)
        issue_firefox_chrome = event_firefox_chrome.issue
        event_firefox_chrome2 = baker.make("issues.Event", issue=issue_firefox_chrome)
        event_firefox_chrome.tags.add(event_tag_firefox)
        event_firefox_chrome2.tags.add(event_tag_chrome)

        event_no_tags = baker.make("issues.Event", issue__project=self.project)

        event_browser_chrome_mythic_animal_firefox = baker.make(
            "issues.Event", issue__project=self.project
        )
